...
void print_matrix(int matrix[N][N]) {
     ...
     *
     * Parameters:
     * @param matrix The given 2D array (matrix).
     * @param n The size of the square matrix (i.e., number of rows/columns).
     */
    
    // Iterate over the rows
	printf("-------------\n");
    for (int i = 0; i < N; i++) {
        // Iterate over the columns to print each cell on stdout
        for (int j = 0; j < N; j++){
            printf("%d ", matrix[i][j]);
