...
/**
 * inicializa los led, los deja apagados y devuelve el número de leds disponibles en la plataforma
 */
...
void drv_led_encender(uint32_t id){
		...
		else if(id == 0) {	//si id es cero enciende todos
...
/**
 * función complementaria a encender
 */
void drv_led_apagar(uint32_t id){
	#if LEDS_NUMBER > 0
		if ((id <= LEDS_NUMBER) && (id >0)) hal_gpio_escribir(led_list[id-1], ~LEDS_ACTIVE_STATE);
		else if(id == 0) {	//si id es cero apaga todos
			for(int i = 0; i< LEDS_NUMBER; i++ ){
					hal_gpio_escribir(led_list[i], ~LEDS_ACTIVE_STATE);
			}
		}
